package com.example.cargoTransportation.logic.kmeans;
...
public class DataSet {
    ...
    public DataSet(List<OrderItem> orders){
        String row;

        for (OrderItem order : orders) {
            String[] attrNames = new String[]{"latitude", "longitude"};
            Double lat = order.getPlace().getLatitude();
            Double lng = order.getPlace().getLongitude();
            if (lat == null || lng == null){
                System.out.println(String.format("ERROR: latitude or longitude not presented for orderItem(id=%s)", order.getId()));
            }else {
                Double[] attrValues = new Double[]{lat, lng};
                HashMap<String, Double> attrRecord = new HashMap<>();

                for (int i = 0; i < attrNames.length; i++) {
                    String name = attrNames[i];
                    Double value = attrValues[i];
                    attrRecord.put(name, value);
                    updateMax(name, value);
                    updateMin(name, value);
                }
                records.add(new Record(attrRecord));
            }
        }
